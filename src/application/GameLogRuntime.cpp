...
namespace gamelog::application
{
    ...
    bool GameLogRuntime::syncGamesWithDatabase()
    {
        ...
        {
            qCWarning(gamelogAgentLog) << "Cannot sync games because GameService or the database is unavailable.";
            return false;
        }
    ...
    bool GameLogRuntime::restoreActiveSession()
    {
        ...
        const std::optional<Session> session = sessionService_->findActiveSession();
        if (!session)
        {
            activeGame_.reset();
            return true;
        ...
        if (!game)
        {
            qCWarning(gamelogAgentLog) << "Active session" << session->id << "references missing game" << session->gameId;
            return false;
        }
    ...
    bool GameLogRuntime::stopActiveSession()
    {
        ...
        {
            qCWarning(gamelogAgentLog) << "Cannot stop the session because SessionService or the active game is unavailable.";
            return false;
        }
